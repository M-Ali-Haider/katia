...
const OTP = ({ email, flow = "login" }) => {
  ...
  const [stateResend, formActionResend] = useFormState(resendOTP, undefined);
  ...
  return (
    <div className="text-white font-inter 3xl:max-w-[500px] max-w-[360px] w-full mt-12 sm:my-20">
      ...
      <form action={formActionOTP}>
        ...
        />
        {stateOTP?.error && (
          <p className="text-[#EC6853] text-xs mt-5">{stateOTP.error}</p>
        )}
        {stateResend?.error && (
          <p className="text-[#EC6853] text-xs mt-5">{stateResend.error}</p>
        )}
        {stateResend?.success && (
          <p className="text-green-500 text-xs mt-5">{stateResend.success}</p>
        )}
      ...
      <form action={formActionResend}>
        ...
        <button
          ...
          className={`mt-5 w-full py-3 px-4 rounded-lg text-sm transition-all duration-200
              ${
                isResendDisabled
